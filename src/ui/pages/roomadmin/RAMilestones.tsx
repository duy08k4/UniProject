...
const RAMilestones: React.FC = () => {
    ...
    const startEditingInfo = () => {
        ...
        if (progress.is_banned) {
            toast.error("Quy trình đã bị đình chỉ, bạn không thể chỉnh sửa thông tin quy trình.")
            return
        }
    ...
    const startAdding = () => {
        if (progress?.is_banned) {
            toast.error("Quy trình đã bị đình chỉ, bạn không thể chỉnh sửa cột mốc.")
            return
        }
    ...
    return (
        <div className="w-full h-fit flex flex-col gap-5 pt-topPadding pb-20">
            {progress.is_banned && (
                <span className="w-fit flex items-center-safe gap-2.5 bg-redRGB py-1.5 px-2 animate-pulse rounded-normal">
                    ...
                    <p className="text-red italic">Quy trình của bạn đã bị đình chỉ bởi Quản trị viên hệ thống.</p>
                </span>
            )}
