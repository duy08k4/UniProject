...
const SDForms: React.FC = () => {
    ...
    const fetchForms = async (p: number = page) => {
        ...
        // Student/Lecturer view: generally active forms (is_stopped=false) and not deleted
        await FormsService.formsPagination(p, pageSize, searchDebounce, false, undefined, classId).finally(() => {
    ...
    return (
        <div className="w-full flex flex-col gap-5 py-mainTwoSidePadding">
            {previewForm && <FormViewer formId={previewForm.id} onClose={() => setPreviewForm(null)} />}

            {/* Header */}
            <div className="w-full flex flex-col justify-center-safe">
                <h1 className="text-largeSize font-bold dark:text-white">Danh sách biểu mẫu</h1>
                <p className="text-gray">
                    Lớp học có tất cả {" "}
                    <b className="text-mainColor">{formPagination?.pagination.total && formPagination?.pagination.total > 9 ? formPagination?.pagination.total : `0${formPagination?.pagination.total}`}</b> {" "}
                    biểu mẫu</p>
            </div>
            ...
            <div className="w-full grid grid-cols-4 gap-5">
                {formPagination?.data.map((form) => (
                    <div key={form.id} className="flex flex-col justify-between gap-3 shadow-[0_0_20px_rgba(128,128,128,0.25)] hover:shadow-[0_0_10px_2px_rgba(128,128,128,0.75)] hoverBtn rounded-normal px-4 py-5 bg-white dark:bg-lightDark">
                        ...
                        <div className="">
                            <p className={`${form.is_stopped ? "text-red" : "text-mainColor"} text-smallSize`}>{form.is_stopped ? "Biểu mẫu đã đóng" : "Biểu mẫu đang mở"}</p>
                        </div>

                        <div className="flex items-center-safe gap-2.5 mt-3 border-t border-gray/10 pt-3">
                            <button
                                className={`flex-1 ${form.is_stopped ? "text-red bg-redRGB" : "text-mainColor bg-mainColorRGB"} px-2.5 py-1.5 rounded-small hoverBtn disableState font-bold text-smallSize`}
                                disabled={isFetching}
