        ...
        <div className="fixed inset-0 z-50 flex bg-black/60">
                ...
                <div className="flex flex-col gap-1 p-3">
                    {topics.map(topic => (
                        <button key={topic.id} onClick={() => setSelected(topic)}
                            ...
                            <p className="font-medium text-smallSize line-clamp-1">{topic.title}</p>
            ...
            <div className="flex-1 flex flex-col bg-white dark:bg-lightDark overflow-hidden">
                {selected && (
                    <>
                        {/* Header */}
                        <div className="px-6 py-4 border-b border-gray/10 flex items-start justify-between gap-4">
                        ...
                        <div className="px-6 py-4 border-t border-gray/10 flex items-center gap-3">
                            ...
                            <button onClick={() => onReview(selected.id, true)} disabled={isFetching}
                                className="px-5 py-2 bg-mainColor text-white rounded-md text-smallSize font-medium hover:opacity-80 transition-opacity disableState shrink-0">
                                Duyệt
                            </button>
                            <button onClick={() => onReview(selected.id, false)} disabled={isFetching}
                                className="px-5 py-2 bg-red text-white rounded-md text-smallSize font-medium hover:opacity-80 transition-opacity disableState shrink-0">
                                Từ chối
                            </button>
                        </div>
                    </>
