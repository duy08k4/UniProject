...
const objectiveIcon: React.ReactNode[] = [
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-8 fill-mainColor">
...
const Home: React.FC = () => {
    return (
        <div className="h-fit flex flex-col gap-40">
            <div className="h-fit flex flex-col gap-5">
                {/* Hero image */}
                <div className="h-fit relative flex justify-center-safe items-center-safe">
                    <div className="h-[400px] aspect-square blur-2xl rounded-full bg-mainColorRGB"></div>
                    <img src={HeroImage} loading="lazy" className="absolute top-1/2 left-1/2 h-[250px] -translate-1/2" />
                </div>

                {/* Hero section */}
                <div className="flex flex-col gap-10 items-center-safe mt-topPadding">
                    <h1 className="uppercase font-bold text-[48px] dark:text-white">hệ thống quản lý đồ án tốt nghiệp</h1>
                    <p className="uppercase text-mainColor text-mediumSize font-light">khoa môi trường và tài nguyên</p>
                    <nav className="flex gap-2.5">
                        <button className="text-mainColor font-bold bg-mainColorRGB px-10 rounded-normal flex items-center-safe gap-2.5 hover:cursor-pointer">
                            Truy cập
            ...
            <div className="h-fit flex flex-col items-center-safe gap-5">
                <div className="">
                    <h1 className="uppercase font-bold text-largeSize dark:text-white">Mục tiêu</h1>
                </div>

                <div className="w-full grid grid-cols-2 gap-y-5 gap-x-5">
                    {objectiveContent.map((value, index) => {
                        return (
                            <span key={index} className="flex gap-2.5 items-center-safe shadow-[0_0_12px_rgba(0,0,0,0.1)] px-5 py-5 rounded-normal dark:bg-lightDark">
                                <span className="bg-mainColorRGB h-[60px] aspect-square flex items-center-safe justify-center-safe rounded-full">{objectiveIcon[index]}</span>
                                <p className="font-semibold text-mediumSize dark:text-white">{value}</p>
                            </span>
                        )
                    })}
                </div>
            ...
            <div className="flex gap-5">
                <div className="w-2/5 flex flex-col gap-10">
                    <div className="flex flex-col gap-2.5">
                        <h1 className="text-hugeSize uppercase font-bold flex items-center-safe gap-2.5 dark:text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-14 fill-mainColor">
                        ...
                        <p className="text-mediumSize font-light ml-3.5 dark:text-lightGray">Các công trình nghiên cứu của sinh viên được lưu trữ tập trung
